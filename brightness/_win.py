"""Windows brightness backend — DDC/CI on SDR displays, SDRWhiteLevel on HDR.
...
The percent<->white-level math lives in `_scale.py`; this module is the Win32
machinery and the monitorcontrol pairing around it.
"""
...
from ._scale import HdrAdjust, sdr_pct_to_wl, sdr_wl_to_pct
...
def compute_hdr_adjust(target: DisplayTarget, delta: int) -> HdrAdjust:
    ...
    signed `delta` (in percent) lands. Shared by the offline CLI (adjust)
